...
const data = [
  {name: '1783 BPS', originalTranscript: '', processedTranscript: '', recording: ''},
  {name: '1811 BOP', originalTranscript: '', processedTranscript: '', recording: ''},
  // other items...
];
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    Voice.onSpeechEnd = () => stopListeningAndRecording();
  ...
  const startRecording = async (itemName) => {
    const path = `${RNFS.DocumentDirectoryPath}/recordings/${itemName}.mp3`;
  ...
  const stopListeningAndRecording = async () => {
    ...
    // Update the processedTranscript and recording path for the selected item
    if (selectedItem) {
  ...
  const handleSelect = item => {
    ...
    setIsProcessedTranscript(!!item.processedTranscript);
    setResult(item.processedTranscript || '');
    
    // Start both listening and recording
    setSelectedItem(item);
    startListening();
    startRecording(item.name);
  };
